...
function sendToWhatsApp() {
  ...
  // Check Login Status
   if (typeof isLoggedIn !== "undefined" && isLoggedIn === false) {
    document.getElementById("message").textContent = "❌ Please log in to submit an order.";
  ...
  //   document.getElementById("sendBtn").style.opacity = 1;
    ...
    let phone = "27827154731";

    let name = document.getElementById("name").value.trim();
    let customerPhone = document.getElementById("customerPhone").value.trim();
    let order = document.getElementById("package").value.trim();
    let from = document.getElementById("pickup").value.trim();
    let to = document.getElementById("dropoff").value.trim();

    //Validation
    if (!name || !customerPhone || !from || !to || !order) {
   ...
   let url = "https://api.whatsapp.com/send?phone=" + phone + "&text=" + message;
    ...
    document.getElementById("message").textContent = "✅ Order prepared for WhatsApp!";
